...
import { Search, User, ListPlus, FolderPlus, FilmIcon, UserCog, Bell, LogIn } from 'lucide-react';
...
const getYear = (releaseDate) => {
  ...
  useEffect(() => {
    ...
    // Make it globally available for updates
      ...
      <nav className="bg-gray-800 p-4 w-full z-50">
        <div className="container mx-auto flex items-center">
          ...
          <div className="flex items-center space-x-4 ml-auto">
              ...
              {filteredMovies.length > 0 && (
                <div className="absolute mt-2 w-[550px] bg-gray-800 text-white rounded-lg shadow-lg max-h-80 overflow-y-auto z-50">
                  {filteredMovies.map((movie) => (
                    ...
                    >
                      <img src={movie.image} alt={movie.title} className="w-16 h-20 rounded-md mr-3" />
                      <div className="flex flex-col">
                        <h3 className="text-lg font-semibold">{movie.title}</h3>
                        <p className="text-sm text-gray-400">{movie.genres.join(' • ')}</p>
                        <p className="text-sm text-gray-400">{movie.year} | {movie.duration} | ⭐ {movie.rating}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {/* Notifications */}
            <Link to="/notifications" className="relative text-white hover:text-red-400 transition-colors">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-lg py-2 border border-gray-700 z-50">
                  <Link to="/userrequests" className="flex items-center px-4 py-2 text-white hover:bg-gray-700 transition-colors">
                    <ListPlus className="w-4 h-4 mr-2" />
                    User Requests
                  </Link>
                  <Link to="/add-movie" className="flex items-center px-4 py-2 text-white hover:bg-gray-700 transition-colors">
